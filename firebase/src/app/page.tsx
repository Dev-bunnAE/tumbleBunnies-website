...
const classIcon = (name: string) => {
  ...
  if (lower.includes("music")) return <Music className="h-10 w-10 text-salmon-dark" />;
...
const PulsingBunny = () => (
    <div className="relative w-6 h-6 mr-2">
        <Rabbit className="w-6 h-6 text-primary-foreground animate-pulse-deep" />
    </div>
);

const HeroSection = memo(function HeroSection() {
    return (
        <section className="relative flex flex-col items-center justify-center w-full min-h-[350px] md:min-h-[420px] bg-gradient-to-b from-bubblegum/30 via-sky/30 via-peach/30 to-salmon/30 py-10 md:py-20 overflow-hidden">
            <div className="container mx-auto px-4 flex flex-col items-center justify-center text-center relative z-10">
                <h1 className="font-headline text-5xl md:text-6xl font-extrabold mb-4 animate-bounce-slow bg-gradient-to-r from-bubblegum-dark via-sky-dark via-peach-dark to-salmon-dark text-transparent bg-clip-text drop-shadow-[0_2px_6px_rgba(0,0,0,0.18)]">
                ...
                <Button
                    size="lg"
                    className="mt-2 px-8 py-4 text-lg font-bold rounded-full shadow-lg bg-salmon border-2 border-bubblegum-dark text-primary hover:bg-salmon-dark hover:text-primary-foreground transition-colors"
                    onClick={() => {
            ...
            <div className="absolute -bottom-16 right-0 w-64 h-64 bg-salmon/20 rounded-full opacity-20 blur-3xl z-0" />
        </section>
    );
});

const WhyTumbleBunniesSection = memo(function WhyTumbleBunniesSection() {
    return (
        <section className="bg-salmon/30 py-16 md:py-24">
                    ...
                    {philosophyPoints.map((point, index) => (
                        <div key={index} className="flex flex-col items-center text-center gap-2">
                            <div className="flex-shrink-0">
                                ...
                                {index === 1 && <Sparkles className="h-8 w-8 text-salmon-dark" aria-label="Safety and skill development" />}
...
const ClassShowcaseSection = memo(function ClassShowcaseSection({ availableClasses }: { availableClasses: any[] }) {
    const [nonagonRadius, setNonagonRadius] = useState(150);
    ...
    useEffect(() => {
        calculateNonagonRadius();
        window.addEventListener('resize', calculateNonagonRadius);
        return () => window.removeEventListener('resize', calculateNonagonRadius);
    }, []);
                ...
                <div className="w-full flex flex-col items-center justify-center">
                    {availableClasses.length === 0 ? (
                        ...
                        <div className="relative mx-auto my-8" style={{ width: 340, height: 340, maxWidth: '90vw', maxHeight: '90vw' }}>
                            <div className="absolute" style={{ left: `calc(50% - 32px)`, top: `calc(50% - 32px)`, zIndex: 10 }}>
                                <Rabbit className="h-16 w-16 text-sky-dark" aria-label="TumbleBunnies Rabbit Icon" />
                            </div>
                            {availableClasses.map((cls, idx) => {
                                const count = availableClasses.length;
                                const angle = (2 * Math.PI * idx) / count;
                                const radius = nonagonRadius;
                                const center = 150;
                                const x = center + radius * Math.cos(angle) - 75;
                                const y = center + radius * Math.sin(angle) - 75;
                                const borderColors = ['border-bubblegum-dark', 'border-salmon-dark', 'border-aqua-dark', 'border-sky-dark', 'border-peach-dark'];
                                const borderColor = borderColors[idx % borderColors.length];
                                return (
                                    <div
                                        key={cls.id}
                                        className={`absolute flex flex-col items-center justify-center w-[150px] h-[150px] p-2 bg-card rounded-xl shadow-xl transition-transform duration-200 hover:-translate-y-1 hover:scale-105 border-4 ${borderColor}`} style={{ left: `${x}px`, top: `${y}px` }}
                                    >
                                        {cls.imageUrl ? (
...
export default function Home() {
            ...
            if (!authLoading) {
                if (user) {
                    if (user.email && user.email.endsWith("@tumblebunnies.com")) {
                        router.replace(user.email === "trainers@tumblebunnies.com" ? "/trainer/attendance" : "/admin");
                        return;
                    }
                    try {
    ...
    async function onFacilityCodeSubmit(values: z.infer<typeof facilityCodeSchema>) {
        ...
        if (facility) {
            router.push(user ? `/register/${facility.id}` : `/register/${facility.id}`);
        } else {
            toast({
                variant: 'destructive',
                title: 'Invalid Code',
    ...
    async function handleGoogleSignIn() {
        ...
        if (currentUser) {
            if (currentUser.email && currentUser.email.endsWith("@tumblebunnies.com")) {
                router.replace(currentUser.email === "trainers@tumblebunnies.com" ? "/trainer/attendance" : "/admin");
                setAuthActionLoading(false);
                return;
            }
            try {
    ...
    async function handleForgotPassword() {
        ...
        try {
            ...
            toast({
                ...
                description: error.code === 'auth/user-not-found' ? 'No account found with this email address.' : 'Failed to send reset email. Please try again.',
            });
        } finally {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background text-center">
            ...
            <main className="flex-grow flex flex-col items-center justify-center">
                            ...
                            {activeForm === 'login' ? (
                                            ...
                                            <Form {...loginForm}>
                                                ...
                                                >
                                                    <FormField
                                                        ...
                                                        render={({ field }) => (
                                                            <FormItem>
                                                                ...
                                                                <FormControl>
                                                                    <Input type="email" placeholder="you@example.com" {...field} className="focus:ring-2 focus:ring-bubblegum" />
                                                                </FormControl>
                                                                <FormMessage />
                                                            </FormItem>
                                                        )}
                                                    ...
                                                    <div className="flex justify-center">
                                                        <Button type="button" variant="link" className="text-sm text-muted-foreground hover:text-primary px-0" onClick={() => setShowForgotPassword(true)}>
                                                            Forgot password?
                                                        </Button>
                                                    </div>
                                                    <Button type="submit" className="w-full" disabled={authActionLoading}>
                                                        {authActionLoading ? <PulsingBunny /> : <LogIn className="mr-2 h-4 w-4" />}
                                                        {authActionLoading ? 'Hopping in...' : 'Sign In'}
                                                    </Button>
                                                </form>
                                            </Form>
                                        ) : (
                                            <div className="space-y-4">
                                                ...
                                                <div className="space-y-4">
                                                    <div>
                                                        ...
                                                        <Input type="email" placeholder="you@example.com" value={forgotPasswordEmail} onChange={(e) => setForgotPasswordEmail(e.target.value)} className="focus:ring-2 focus:ring-bubblegum" />
                                                    </div>
                                                    <div className="flex gap-2">
                                                        <Button type="button" variant="outline" className="flex-1" onClick={() => { setShowForgotPassword(false); setForgotPasswordEmail(''); }} disabled={forgotPasswordLoading}>
                                                            Back to Login
                                                        </Button>
                                                        <Button type="button" className="flex-1" onClick={handleForgotPassword} disabled={forgotPasswordLoading}>
                                                            {forgotPasswordLoading ? 'Sending...' : <><Mail className="mr-2 h-4 w-4" /> Send Reset Link</>}
                                                        </Button>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <>
                                    <CardHeader className="text-center">
                                        <CardDescription>Enter your facility code to get started.</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                                        <Form {...facilityCodeForm}>
                                            <form onSubmit={facilityCodeForm.handleSubmit(onFacilityCodeSubmit)} className="space-y-4">
                                                <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            ...
                                                            <FormControl>
                                                                <Input placeholder="Facility Code" {...field} className="text-center" />
                                                            </FormControl>
                                                            <FormMessage />
                                                        </FormItem>
                                                    )}
                                                />
                                                <Button type="submit" className="w-full" disabled={authActionLoading}>
                                                    {authActionLoading ? 'Checking...' : <><UserPlus className="mr-2 h-4 w-4" /> Register</>}
                                                </Button>
                                            </form>
                                        </Form>
                                    </CardContent>
                ...
                <WhyTumbleBunniesSection />
                <ClassShowcaseSection availableClasses={availableClasses} />
            </main>
            <footer className="bg-primary text-primary-foreground py-6 border-t-4 border-gradient-to-r from-bubblegum-dark via-salmon-dark via-sky-dark to-peach-dark">
                <div className="container mx-auto px-4 flex justify-center items-center">
                    <div className="flex gap-6">
                        <a href="https://www.facebook.com/tumblebunnies/" target="_blank" rel="noopener noreferrer" aria-label="TumbleBunnies on Facebook" className="rounded-full bg-white border-2 border-bubblegum-dark p-1 shadow hover:bg-bubblegum/20 transition">
                        ...
                        <a href="https://www.instagram.com/tumblebunnies/?hl=en" target="_blank" rel="noopener noreferrer" aria-label="TumbleBunnies on Instagram" className="rounded-full bg-white border-2 border-bubblegum-dark p-1 shadow hover:bg-bubblegum/20 transition">
